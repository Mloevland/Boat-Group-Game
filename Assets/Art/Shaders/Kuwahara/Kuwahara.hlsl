void Kuwahara_float(UnityTexture2D blit,UnitySamplerState ss, float2 uv, float2 viewsize,float2 radius, float depth, out float3 Out) 
{
	//Kuwahara Variables
	float3 mean[4] = {
		{0,0,0},
	...
	float2 texelSize = depth / viewsize;
	...
	for (int x = -1; x <= 1; x++)
	{
		...
		for (int y = -1; y <= 1; y++)
		{
			...
			float2 offset = float2(x, y) * texelSize *3;
	...
	//Out = float3(gradientX, gradientY,0);
	Out = col;
